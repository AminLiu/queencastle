(function($){ 
	$.fn.touchslide = function(options){ 
		var defaults = { 
			timecontrol:1000,//图片停留时间
			animatetime:200, //图片滑动所需时间
			direction:'left', //轮播方向
			navshow:true,//是否显示图片导航栏
			canvassuport:true//图片导航栏是否开启cavas绘制圆
		} 
		var options = $.extend(defaults, options); 
		var timecontrol = options.timecontrol||1000;
		var animatetime = options.animatetime||200;
			...
			if(imgcount>1){
				...
				if(navshow){
					...
					for(var i=0;i<imgcountrealy;i++){
						circlewrapper.append("<canvas class='circle' width='8' height='8'></canvas>");
					}
